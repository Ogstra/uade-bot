...
import (
	...
	"database/sql"
	"errors"
	"github.com/Ogstra/uade-bot/internal/app"
	"github.com/Ogstra/uade-bot/internal/cutover"
	...
	if token != "" && mode != "shadow" {
		...
		client.AddEventListeners(bot.NewListenerFunc(func(*events.GuildsReady) {
			go func() {
				...
				log.Printf("converged 16 global discord commands and cleared guild-scoped copies in %d guilds", len(guildIDs))
			}()
		}))
